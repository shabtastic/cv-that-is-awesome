...
% Before each \cventry: if previous entry had a description (cvitems), add
% 8pt extra clearance so the bullet text (which extends below the tabular's
% measured height) does not overlap the current entry's header.
% For plain->plain transitions, 4pt matches the original cls spacing.
\pretocmd{\cventry}{%
  \ifbool{firstcventry}{%
    \boolfalse{firstcventry}%
  }{%
    \ifbool{prevhaddesc}{%
      \vspace{6pt}%  % extra: clears cvitems overflow (was 8pt)
    }{%
      \vspace{4pt}%  % normal inter-entry gap
    }%
  }%
  \boolfalse{prevhaddesc}%
  \noindent%
...
% cvitems: closing \vspace tuned empirically.
%
% The closing vspace is INSIDE the \multicolumn cell of \cventry's tabular.
% Contrary to earlier assumption, it DOES propagate to inter-entry spacing by
% inflating the tabular row height. A sufficiently negative value (~-10.5mm)
% compensates for the bullet-list height and brings bullet-entries into
% alignment with plain entries (~15pt gap).
%
% \iftoggle-wrapped entries add a paragraph break on exit (~13.5pt extra).
% These are patched per-section with explicit \vspace{-13.5pt} after }{}.
% Any \cventry with bullets followed by another entry needs \vspace{-11.8pt}
% between them if the cvitems closing vspace is insufficient (e.g. single-item
% lists whose tabular height floor is hit before the vspace takes effect).
