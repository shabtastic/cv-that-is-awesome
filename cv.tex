...
%
% Entries that go through the normal name formatter (no authorlist field)
% will have "Hakimi" bolded automatically. Entries with an authorlist field
...
% NOTE: entries using the `authorlist` field bypass the name formatter.
% Bold your name manually there, e.g.: authorlist = {\textbf{Hakimi, S}, ...}

%------------------------------------------------------------------------------
% AUTHORLIST FIELD
%
% Custom verbatim field for pre-formatted author strings with LaTeX commands
% (\textbf, \cvfnmark, etc.). Declared verbatim so biber does not sanitise it.
% The author macro checks for this field and uses it instead of \printnames.
%------------------------------------------------------------------------------
% authorlist: custom verbatim field for pre-formatted author strings.
% Declared for all common entry types so biber passes it through unmodified.
\DeclareDatamodelFields[type=field, datatype=verbatim]{authorlist}
\DeclareDatamodelEntryfields[article,inproceedings,unpublished,misc,patent,incollection]{authorlist}
...
%------------------------------------------------------------------------------
% NOTE FIELD SUPPRESSION
%
% The note field is used in some .bib entries for equal-contribution text
% (e.g. "* equal contributions"). We suppress it from printing inline since
% that information is handled by \cvfnmark in the authorlist field instead.
%
% Exception: @misc entries (Science Communication) may use note legitimately.
% We restore note printing for @misc via a per-type field format override.
%------------------------------------------------------------------------------
\DeclareFieldFormat{note}{}
\renewbibmacro*{note+pages}{%
  \printfield{pages}%
}
% Restore note for @misc entries (scicomm uses it for publication details)
