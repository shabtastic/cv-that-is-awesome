...
on record["patentNumber"]. ODP's applicationMetaData never populates that field
for these applications -- it was empty for all 15 filed entries on 2026-08-16,
including app 17825648, whose status was "Patented Case" with an "Issue
Notification Mailed" event. So the check could never fire, and reported
"No status changes found" as a false negative.

The authoritative signal is applicationStatusDescriptionText. The fixtures below
are the literal status strings returned by ODP for this bib's applications.
"""
...
def test_patented_case_without_patent_number_reports_none():
    """ODP marks the case patented before it publishes the grant number."""
    state, num = classify_app_status(rec("Patented Case"))
    assert state == "granted"
    assert num is None
