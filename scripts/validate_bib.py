...
from _shared import _extract_braced_field  # noqa: E402
...
def check_entry(entry: str, file_name: str, report: Report) -> dict:
    ...
    # 3. DOI format sanity
    doi = _extract_braced_field(entry, "doi").strip()
    if doi and not re.match(r"^10\.\d{3,}/", doi):
...
def check_cross_file(all_meta: list[dict], report: Report) -> None:
    """Duplicate cite keys and DOIs across files."""
