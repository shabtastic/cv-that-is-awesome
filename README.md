...
```
cv/
├── cv.tex                      # Master file — preamble, section layout, \input{cv-preset}
├── cv-preset.tex               # Active preset (auto-generated by make; default = full)
...
├── config/                     # Named build presets (edit these, not cv.tex)
│   ├── full.tex                # Full academic CV — all sections, no filters
│   ├── industry.tex            # Industry/job application — all pubs, lean service
│   └── speaking.tex            # Speaking/bio — talks forward, selected pubs
├── sections/
│   ├── education.tex
│   ├── experience.tex
│   ├── grants.tex
...
│   ├── teaching.tex
│   ├── service.tex             # \cventryselected = always shown; rest gated by showAllService
│   ├── reviewer.tex
│   ├── mentorship.tex
│   └── memberships.tex
├── refs/
...
### Named preset builds (recommended):
```bash
make full        # → cv-full.pdf      (complete academic CV)
make industry    # → cv-industry.pdf  (all pubs, lean service, no grants/teaching)
make speaking    # → cv-speaking.pdf  (talks forward, selected pubs)
make all         # → builds all three
```

Each target copies the corresponding `config/*.tex` file to `cv-preset.tex`, runs the full XeLaTeX + Biber compile sequence, and renames the output.

### Manual build (custom preset):
```bash
cp config/full.tex cv-preset.tex   # or any config file
make pdf                           # compiles → cv.pdf
```

### Manual build sequence (no make):
```bash
xelatex cv.tex
biber cv
xelatex cv.tex
...
Toggle values are no longer set directly in `cv.tex`. Instead, they live in preset files under `config/`. The active preset is loaded via `\input{cv-preset}` in `cv.tex`.

| Preset | File | Output | Description |
|---|---|---|---|
| Full academic | `config/full.tex` | `cv-full.pdf` | All sections, all entries, no filters |
| Industry | `config/industry.tex` | `cv-industry.pdf` | All pubs unfiltered, talks + service condensed, no grants/teaching |
| Speaking/bio | `config/speaking.tex` | `cv-speaking.pdf` | Talks front-and-center, selected pubs only |

`cv-preset.tex` at the repo root is the committed fallback (a copy of `config/full.tex`). The Makefile overwrites it at build time. A fresh clone compiles cleanly with `make pdf` without needing to run a preset target first.

---

## Toggle Reference

Toggles are set in preset files (`config/*.tex`), not in `cv.tex` directly. To change a toggle for a specific preset, edit the corresponding file in `config/`. To change it globally, edit all three presets or update `cv-preset.tex` directly for a one-off build.

### Section visibility

| Toggle | Effect |
|---|---|
| `showJournals` | Peer-reviewed journal publications |
| `showPreprints` | Preprints & working papers |
| `showConferenceProceedings` | Conference proceedings |
| `showConferencePresentations` | Selected conference presentations |
| `showPatents` | Patents section |
| `showSciComm` | Science communication subsection |
| `showBookChapters` | Book chapters |
| `showTalks` | Invited talks subsection |
| `showGrants` | Grants & Awards section |
| `showTeaching` | Teaching section |
| `showService` | Professional Service section |
| `showReviewer` | Reviewing section |
| `showMentorship` | Mentorship section |
| `showMemberships` | Memberships section |
| `showReferences` | References section |
| `showFullReferences` | `true` = list referees; `false` = "Available upon request" |
...
| Toggle | Effect |
|---|---|
| `selectedOnlyPublications` | Changes section heading to "Selected Publications" |
| `selectedOnlyJournals` | Filter journals to `keywords={selected}` only |
| `selectedOnlyPreprints` | Filter preprints |
| `selectedOnlyConferenceProceedings` | Filter conference proceedings |
| `selectedOnlyPresentations` | Filter presentations |
| `selectedOnlyPatents` | Filter patents (mirrors `\patentFilter`) |
...
| Toggle | Effect |
|---|---|
| `showAllTalks` | `true` = all 9 talks; `false` = 6 condensed ("Selected Invited Talks") |
| `showAllGrants` | `true` = show all grants |
| `showAllTeaching` | `true` = show all teaching entries |
| `showAllService` | `true` = all service entries; `false` = `\cventryselected` entries only |

### Bibliography numbering

| Toggle | Effect |
|---|---|
| `showBibNumbers` | Show [1], [2], … labels before each entry |
| `reverseBibNumbers` | `true` = N…1 (most recent first); `false` = 1…N |

### Patent display mode
Patents use `\def` rather than toggles:
```latex
\def\patentMode{none}       % omit patents section entirely
\def\patentMode{granted}    % show granted patents only
\def\patentMode{all}        % show granted + filed
